...
function init {
    make &>/dev/null
    if [ $? -ne 0 ]; then
      echo "Makefile not found or 'make' failed!"
      exit 1
    fi
...
function run_test {
    ...
    # Run program.
    if [ $1 -eq 20 ]; then
        valgrind --log-file="leak-check" --leak-check=full ./$PAINT_EXEC <$input_file 2&>$output_file
        rc=$?

        if ! grep -q 'no leaks are possible' leak-check; then
            memory_leaks=true
            echo -e "Memory leaks detected!"
        fi
    else
        ./$PAINT_EXEC <$input_file 2&>$output_file
        rc=$?
    fi
    ...
    if [ $? -ne 0 ] || $memory_leaks; then
        echo -e "TEST $1  \e[31mFAILED\e[0m ... 0 points"
    else
        ...
        echo -e "TEST $1  \e[32mPASSED\e[0m ... " $points "points"
        total=$((total + points))
    fi
...
function clean {
    ...
    rm -rf output_ppm &>/dev/null
    rm -rf leak-check &>/dev/null
